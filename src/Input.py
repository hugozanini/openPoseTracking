...
class Input():
    ...
    def run_local(self, ret, frame, frames_array):
            ...
            for track in self.tracker.tracks:
                ...
                (text_width, text_height) = cv2.getTextSize(text,0, fontScale= 5e-3 * 150, thickness=2)[0]
                # set the text start position
                text_offset_x = int(bbox[0])
                text_offset_y = int(bbox[1])-20
                box_coords = ((text_offset_x, text_offset_y), (text_offset_x + text_width - 2, text_offset_y - text_height - 2))
                cv2.rectangle(self.currentFrame, box_coords[0], box_coords[1], color_ret, cv2.FILLED)

#               cv2.rectangle(self.currentFrame, (int(bbox[0]), int(bbox[1])-20), (0, 1), (255,255,255),2)
                cv2.putText(self.currentFrame, text,(int(bbox[0]), int(bbox[1])-20),0, 5e-3 * 150, color,2)
